...
Part 4: STATIC ESTIMATION
Step 2: MiniMaxGameImproved
'''
...
def NextMove(board, d=0):
    ...
    if d == depth:
        return board.StaticEstimation_MidgameEndgame_Improved()
    
    # Base Case 2 (white count = 2 or black count = 2)
    if d != 0 and (board.whiteCount() == 2 or board.blackCount() == 2):
        return board.StaticEstimation_MidgameEndgame_Improved()
